...
{
    [SerializeField] GridObjectPrefab gridObjectPrefab;
    ...
    void Start()
    {
        ...
        gridObjectPrefab.Setup (map.grid);
        pathfinding.SetNodes (map.objects);
    }
    ...
    void Update()
    {
        if (Input.GetMouseButtonUp (0))
		{
            ...
            pathfinding.SetStartPosition (position, map.grid);
		}

        if (Input.GetMouseButtonUp (1))
        {
            Vector3 position = Camera.main.ScreenToWorldPoint (Input.mousePosition);
            pathfinding.SetEndPosition (position, map.grid);
            pathfinding.GetShortestPath();
            panel.UpdateValues (pathfinding.start.ToString(), pathfinding.end.ToString(), pathfinding.path.transfers, pathfinding.path, pathfinding.path.length);
        }
    }
